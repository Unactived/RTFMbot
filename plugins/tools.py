...
class Tools:
    ...
    async def ascii(self, ctx, *, text: str):
        ...
        await ctx.send(','.join([ord(letter) for letter in text.strip('``').strip('`')]))
    ...
    async def unascii(self, ctx, *, text: str):
        ...
        try:
            ...
            await ctx.send(f"Invalid sequence. Example usage : \
                `{self.bot.config['PREFIX']}unascii 104,101,121`")

    @commands.command()
    async def byteconvert(self, ctx, value, unit='Mio'):
